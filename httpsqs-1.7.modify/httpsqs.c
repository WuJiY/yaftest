...
char *httpsqs_settings_whitelistfile = NULL;
char *httpsqs_settings_blacklistfile = NULL;
...
void httpsqs_handler(struct evhttp_request *req, void *arg)
{
    ...
    const char *httpsqs_input_ip = evhttp_find_header(&httpsqs_http_query, "ip"); /* ip地址*/

    char ip_address[16];
    char tmp_ip_address[16];
    FILE *wfp *bfp;
    // 有白名单
    if (httpsqs_settings_whitelistfile != NULL) {
        wfp = fopen(httpsqs_settings_whitelistfile,'r');
        if (wfp == NULL) {
            evbuffer_add_printf(buf, "%s", "HTTPSQS_FOPEN_WHITELIST_ERROR");
        }
        while (fgets(ip_address, 16, wfp) != NULL) {
            memset(tmp_ip_address, 0x00,sizeof(char) * 16); 
            strncpy(tmp_ip_address, ip_address, strlen(ip_address) - 1);
            if (strcmp(tmp_ip_address, httpsqs_input_ip) == 0) {
                break;
            }
            //  返回错误
        }
    ...
    // 有很名单
    if (httpsqs_settings_blacklistfile != NULL) {
        bfp = fopen(httpsqs_settings_blacklistfile, "r");
        if (bfp == NULL) {
            evbuffer_add_printf(buf, "%s", "HTTPSQS_FOPEN_BLACKLIST_ERROR");
        }
        while (fgets(ip_address, 16, bfp) != NULL) { 
            memset(tmp_ip_address, 0x00, sizeof(char) * 16);
            strncpy(tmp_ip_address, ip_address, strlen(ip_address) -1);
        }
...
int main(int argc, char *argv[], char *envp[])
{
    ...
    if (readlink("/proc/self/exe", httpsqs_path_file, sizeof(httpsqs_path_file)) < 0){
        printf("get error");
        return 1;
    } 
    ...
    /* process arguments */
    while ((c = getopt(argc, argv, "l:p:x:w:t:s:c:m:i:a:dh")) != -1) {
        switch (c) {
            case 'l':
            ...
            case 'x':
                ...
                break;
            case 'w':
                httpsqs_settings_whitelistfile = strdup(optarg);
                if (access(httpsqs_settings_whitelistfile, R_OK) != 0) {
                    fprintf(stderr, "httpsqs white list file not readable\n");
                }
                break;
            case 'b':
                httpsqs_settings_blacklistfile = strdup(optarg);
                if (access(httpsqs_settings_blacklistfile, R_OK) != 0){
                    fprintf(stderr, "httpsqs black list file not readable\n");
                }
                break;
            case 't':
    ...
    /* 判断是否有白名单 -w , 如果有白名单，则缓存*/
    if (httpsqs_settings_whitelistfile != NULL) {

    }
    /* 判断是否添加有黑名单过滤, 如果有黑名单，则缓存过滤*/
    if (httpsqs_settings_blacklistfile != NULL) {
